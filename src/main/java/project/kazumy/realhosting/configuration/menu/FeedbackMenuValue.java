package project.kazumy.realhosting.configuration.menu;
...
@ConfigSection("menu")
@ConfigFile("menu.yml")
public class FeedbackMenuValue implements ConfigurationInjectable {

    @Getter private static final FeedbackMenuValue instance = new FeedbackMenuValue();

    @ConfigField("feedback") private ConfigurationSection menu;

    @SneakyThrows
    public SelectMenu toMenu() {
        val yamlFile = new YamlFile("configuration/menu.yml");
        yamlFile.load();
        val menu = StringSelectMenu.create("ticket-feedback-menu");
    ...
    public static <T> T get(Function<FeedbackMenuValue, T> function) {
