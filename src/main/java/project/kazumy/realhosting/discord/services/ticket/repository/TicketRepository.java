...
public class TicketRepository {
    ...
    public void save(Ticket ticket) {
        executor.updateQuery("INSERT INTO ticket (name, owner, category, chat, closed, participants, history, creation, cancellation)" +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", statement -> {
            ...
            statement.set(6, TicketUtils.participantsToJson(ticket.getParticipants()));
            statement.set(7, TicketUtils.historyToJson(ticket.getHistory()));
            statement.set(8, ticket.getCreation().toString());
            statement.set(9, ticket.getCancellation() == null ? "" : ticket.getCancellation().toString());
        });
    }

    public void close(Ticket ticket) {
        executor.updateQuery("INSERT INTO ticket (id, name, owner, category, chat, closed, participants, history, creation, cancellation)" +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", statement -> {
            ...
            statement.set(10, LocalDateTime.now(ZoneId.of("America/Sao_Paulo")));
        });
    }
