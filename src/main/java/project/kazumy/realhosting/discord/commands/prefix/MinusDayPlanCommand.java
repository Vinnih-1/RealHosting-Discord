...
public class MinusDayPlanCommand extends BasePrefixCommand {

    public MinusDayPlanCommand() {
        super("minusday", Permission.ADMINISTRATOR);
    }
    ...
    public void execute(Member member, Message message, String[] args) {
        ...
        try {
            ...
            plan.setExpirationDate(plan.getExpirationDate().minusDays(Long.parseLong(args[1])));
            plan.saveConfig();

            channel.sendMessageEmbeds(new EmbedBuilder()
                            .setColor(Color.GREEN)
                            .setDescription(String.format("<@%s>, você removeu **%s dia(s)** ao plano %s, pertencente ao usuário <@%s>",
