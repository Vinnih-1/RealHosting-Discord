...
public class SubUserButtonInteraction extends InteractionService<ButtonInteractionEvent> {

    public SubUserButtonInteraction() {
        super("subuser");
    }
        ...
        event.getMessage().getEmbeds().get(0).getFields().stream()
                ...
                .ifPresentOrElse(field -> {
                    ...
                    event.deferReply(true).addEmbeds(new EmbedBuilder()
                            ...
                            .setDescription("Para adicionar um sub-usuário no seu servidor, utilize o comando `/subusuario`.")
                            .build()).queue();
                }, () -> {
                    event.deferReply(true).addEmbeds(new EmbedBuilder()
                                    .setColor(Color.RED)
